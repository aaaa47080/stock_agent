...
from utils.utils import DataFrameEncoder, get_crypto_news, safe_float
...
class MarketPulseAnalyzer:
    ...
    def __init__(self, client=None):
        ...
        if client is None:
            try:
                self.client, self.model = create_llm_client_from_config(
                    MARKET_PULSE_MODEL
                )
                ...
                if "Missing OpenAI API Key" in str(e):
                    logger.info(
                        "MarketPulseAnalyzer initialized in fallback mode (no system OpenAI API key)."
                    )
                else:
                    logger.warning(
                        f"MarketPulseAnalyzer initialized WITHOUT LLM (Fallback Mode): {e}"
                    )
                self.client = None
                self.model = None
        else:
    ...
    def analyze_movement(
        ...
        try:
            ...
            macd_hist_col = next(
                (c for c in df.columns if c.startswith("MACDh_")), None
            )
            ...
            vol_status = (
                f"High ({vol_ratio:.1f}x)"
                if vol_ratio > 1.5
                else ("Low" if vol_ratio < 0.7 else "Normal")
            )
        ...
        news_data = get_crypto_news(
            symbol, limit=news_limit, enabled_sources=enabled_sources
        )
        ...
        if self.client and not skip_llm:
            report = self._generate_structured_report(
                symbol, current_price, change_1h, change_24h, technicals, news_data
            )
        else:
            ...
            report = {
                ...
                "key_points_i18n": [
                    ...
                    {
                        "key": "techSignals",
                        "values": {"macd": macd_trend, "volume": vol_status},
                    },
                ...
                "highlights": [
                    {
                        "title_key": "techOverview",
                        "title": "Technical Overview",
                        "content": f"RSI: {rsi_val:.1f}, MACD: {technicals.get('MACD_Signal', 'N/A')}",
                    }
                ],
                "risks_i18n": [risk_key],
                "risks": [risk_key],
        ...
        try:
            ...
            return {
                ...
                "highlights": [
                    {
                        "title": "價格表現",
                        "content": f"過去24小時變動 {change_24h:.2f}%，目前處於盤整階段。",
                    },
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            get_market_pulse("BTC"), indent=2, ensure_ascii=False, cls=DataFrameEncoder
        )
    )
