...
def screen_top_cryptos_light(
    exchange="okx",
    limit=10,
    interval="1d",
    target_symbols: Optional[List[str]] = None,
    market_pulse_data: dict = None,
):
    ...
    # Check cache
    now = time.time()
    if exchange in _TICKER_CACHE and (
        now - _TICKER_CACHE[exchange]["timestamp"] < CACHE_SLAPSHOT_DURATION
    ):
        ...
        try:
            ...
            # Update cache
            _TICKER_CACHE[exchange] = {"data": usdt_tickers, "timestamp": now}
        except Exception as e:
    ...
    df_gainers = (
        df_liquid.sort_values(by="price_change_24h", ascending=False).head(5).copy()
    )
    df_losers = (
        df_liquid.sort_values(by="price_change_24h", ascending=True).head(5).copy()
    )
...
if __name__ == "__main__":
    ...
    start = time.time()
    summary, top, oversold, overbought = screen_top_cryptos_light(
        exchange="okx", limit=10
    )
    elapsed = time.time() - start
    print(f"\nElapsed time: {elapsed:.2f}s")
    print(
        f"Results: {len(top)} total, {len(oversold)} oversold, {len(overbought)} overbought"
    )
