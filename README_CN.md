...
| 優勢 | 說明 |
|------|------|
| 🧠 **Agent V4 智能架構** | 涵蓋加密貨幣、美股、台股之互動式多市場分析 |
| 💬 **PTT 風格社群** | 專業投資討論區，原生支援 Pi 幣打賞 |
| 💰 **Pi 生態整合** | 發文、打賞、會員升級全視角 Web3 支付 |
| 🔐 **隱私優先設計** | 用戶數據自主，安全交易 |

### 獨特賣點

- **市場首個**整合 Pi Network 的開放前瞻 AI 分析平台
- **互動式 Agent V4 系統** — Human-in-the-loop (HitL) 智能規劃與深度討論機制
- **全方位市場支援** — 無縫追蹤加密貨幣、美股 (US Stocks) 與台股 (TW Stocks)
- **完整社群生態** — 知識討論 + 社交互動 + 市場交易
- **即時多交易所數據** — 高效的 OKX + Binance 統一接口
...
| 代理 | 職責 |
|------|------|
| **Crypto Agent** | 加密貨幣基本面、鏈上數據、即時報價與 Web3 新聞分析 |
| **US Stock Agent** | 美股 (NYSE/NASDAQ) 市場資訊、財報分析與企業動態 |
| **TW Stock Agent** | 台股上市櫃資料、三大法人動向與在地新聞 |
| **Manager Agent** | 智能意圖分類、執行計畫編排與使用者互動協調 |

**互動式執行流程 (Human-in-the-loop)**

```
用戶複雜查詢 → [Manager 代理進行意圖分類]
                      ↓
           [系統自動預先收集基礎數據]
                      ↓
    [提出多步驟分析計畫] ↔ 用戶可參與討論或修改計畫
                      ↓
    [各 Agent 依照計畫執行：技術/籌碼/基本面等]
                      ↓
              [產出最終綜合報告]
```
