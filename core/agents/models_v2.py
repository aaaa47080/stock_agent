...
1. 開放式意圖理解 - 不使用硬編碼類別
2. DAG 任務圖 - 支援垂直/水平執行
3. 選擇性上下文傳輸 - Sub-Agent 只接收必要資訊
4. 短期記憶 - 對話歷史壓縮與符號快取

與 V1 差異：
- 無 TaskComplexity 枚舉（改為動態判斷）
- 無預設 intent 類別
- 新增 TaskNode/TaskGroup 支援 DAG
- 新增 ShortTermMemory 結構
- 新增 AgentContext 選擇性傳輸
"""
...
class ManagerStateV2(TypedDict, total=False):
    ...
    與 V1 差異：
    - 移除 complexity（改用 execution_mode）
    - 移除 intent 字串（改用 IntentUnderstanding 結構）
    - 新增 task_graph
    - 新增 short_term_memory
    """
    # === 必填 ===
    session_id: str
    query: str
