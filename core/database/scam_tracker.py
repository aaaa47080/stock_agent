...
def create_scam_report(
    ...
    try:
        ...
        try:
            AuditLogger.log(
                action="create_scam_report",
                user_id=reporter_user_id,
                resource_type="scam_report",
                resource_id=str(report_id),
                endpoint="/api/scam-tracker/reports",
                method="POST",
                success=True,
                cursor=c,
            )
        ...
        try:
            ...
            logger.warning(
                f"Audit log failed after report creation error: {audit_error}"
            )
...
def get_scam_report_by_id(
    report_id: int, increment_view: bool = True, viewer_user_id: Optional[str] = None
) -> Optional[Dict]:
    """
    獲取舉報詳情
...
def vote_scam_report(report_id: int, user_id: str, vote_type: str) -> Dict:
    """
    對舉報投票（支持 Toggle 切換）
    ...
    try:
        # 檢查舉報是否存在
        c.execute(
            "SELECT reporter_user_id FROM scam_reports WHERE id = %s", (report_id,)
        )
...
def add_scam_comment(
    report_id: int, user_id: str, content: str, transaction_hash: Optional[str] = None
) -> Dict:
    """
    添加評論（僅 Premium 用戶）
...
def get_scam_comments(report_id: int, limit: int = 50, offset: int = 0) -> List[Dict]:
    """
    獲取評論列表
