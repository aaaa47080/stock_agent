...
def send_message(
    from_user_id: str, to_user_id: str, content: str, message_type: str = "text"
) -> Dict:
    """
    發送訊息（優化版：單一資料庫連接）
    """
...
def get_messages(
    conversation_id: int, user_id: str, limit: int = 50, before_id: Optional[int] = None
) -> Dict:
    """
    取得對話中的訊息（分頁）
    """
    ...
    try:
        ...
        return {"success": True, "messages": messages, "has_more": len(rows) == limit}
    finally:
        conn.close()
...
def mark_as_read(conversation_id: int, user_id: str) -> Dict:
    ...
    try:
        ...
        if conv["user1_id"] == user_id:
            c.execute(
                "UPDATE dm_conversations SET user1_unread_count = 0 WHERE id = %s",
                (conversation_id,),
            )
        else:
            c.execute(
                "UPDATE dm_conversations SET user2_unread_count = 0 WHERE id = %s",
                (conversation_id,),
            )
