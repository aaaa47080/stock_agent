...
def create_or_get_pi_user(
    pi_uid: str, username: Optional[str] = None, wallet_address: Optional[str] = None
) -> Dict:
    """
    創建或獲取 Pi Network 用戶
    ...
    try:
        # 檢查 pi_uid 是否已存在
        c.execute(
            "SELECT user_id, username, auth_method, pi_username, role, membership_tier FROM users WHERE pi_uid = %s",
            (pi_uid,),
        )
        row = c.fetchone()
        if row:
            ...
            if updates:
                ...
                c.execute(
                    f"UPDATE users SET {', '.join(updates)} WHERE pi_uid = %s", params
                )
                conn.commit()
            return {
                "user_id": row[0],
...
def get_user_by_pi_uid(pi_uid: str) -> Optional[Dict]:
    ...
    try:
        c.execute(
            "SELECT user_id, username, auth_method, created_at FROM users WHERE pi_uid = %s",
            (pi_uid,),
        )
        row = c.fetchone()
        if row:
...
def upgrade_to_pro(
    ...
    try:
        ...
        c.execute(
            "SELECT membership_tier, membership_expires_at FROM users WHERE user_id = %s",
            (user_id,),
        )
        row = c.fetchone()
