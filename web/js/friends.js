...
const FriendsUI = {
            ...
            return `
                <div class="flex gap-2">
                    <button onclick="event.stopPropagation(); event.preventDefault(); FriendsUI.handleRemoveFriend('${encodeURIComponent(userId)}')"
                        ...
                        <span class="group-hover:hidden">${t('friends.friend')}</span>
    ...
    renderUserCard(user, showActions = true) {
        ...
        const actionBtn = showActions
            ? this.getFriendButton(user.user_id, user.friend_status, user.is_requester)
            : '';
                ...
                <a href="/static/forum/profile.html?id=${user.user_id}" onclick="sessionStorage.setItem('returnToTab', 'friends')" class="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <div class="text-xs text-textMuted">
                            ...
                            ${user.sent_at ? (t('friends.requestSentLabel')) + this.formatTime(user.sent_at) : ''}
                        </div>
                    </div>
                </a>
    ...
    async handleAddFriend(userId) {
        ...
        try {
            ...
            if (btn) {
                ...
                btn.disabled = false;
                btn.className =
                    'friend-btn bg-white/5 text-textMuted px-3 py-1.5 rounded-lg text-sm font-bold flex items-center gap-1 border border-white/10 hover:bg-danger/10 hover:text-danger hover:border-danger/20 transition';
    ...
    async handleUnblock(userId) {
        try {
            ...
            if (typeof showToast === 'function') {
                showToast(t('friends.userUnblocked'), 'success');
            }
            // Refresh in-place without reloading
            await loadFriendsTabData();
        } catch (error) {
    ...
    toggleBlockedView() {
        ...
        const toggleBtnSpan = document.querySelector(
            'button[onclick="FriendsUI.toggleBlockedView()"] span'
        );
...
const SocialHub = {
    ...
    loadConversations: async function () {
        ...
        try {
            ...
            const res = await AppAPI.get(`/api/messages/conversations?limit=20&user_id=${myId}`);
            console.log('[SocialHub] Conversations loaded:', res);
            ...
            if (!res.conversations || res.conversations.length === 0) {
                ...
                listEl.innerHTML = res.conversations
    ...
    renderConversationItem: function (conv) {
        ...
        const lastMessage = conv.last_message || (t('friends.clickToChat'));
    ...
    loadMessages: async function (userId) {
        ...
        try {
            ...
            console.error('載入訊息失敗:', e);
                container.innerHTML = `<div class="text-center text-danger py-4 text-sm">${SecurityUtils.escapeHTML(e.message || (t('friends.loadFailed')))}</div>`;
        }
    ...
    renderMessageBubble: function (msg) {
        ...
        if (isRecalled) {
            const recalledText = isMe
                ? (t('messages.recalledByMe'))
                : (t('messages.recalledByOther'));
            return `
        ...
        const msgActions = isMe
            ...
            <div class="absolute ${isMe ? '-left-16' : '-right-16'} top-1/2 -translate-y-1/2 flex gap-1 opacity-0 group-hover:opacity-100 transition">
                <button onclick="event.stopPropagation(); SocialHub.recallMessage(${msg.id}, this)"
                        class="p-1 text-textMuted/50 hover:text-warning transition disabled:opacity-50 disabled:cursor-not-allowed" title="${t('messages.recallTitle')}">
                    <i data-lucide="undo-2" class="w-3.5 h-3.5"></i>
                </button>
                <button onclick="event.stopPropagation(); SocialHub.hideMessage(${msg.id}, this)"
                        class="p-1 text-textMuted/50 hover:text-danger transition disabled:opacity-50 disabled:cursor-not-allowed" title="${t('friends.deleteHint')}">
                    <i data-lucide="trash-2" class="w-3.5 h-3.5"></i>
                </button>
            </div>
        `
    ...
    recallMessage: async function (messageId, btnElement) {
        if (!confirm(t('messages.recallConfirm'))) return;
        ...
        try {
            ...
            if (data.success) {
                ...
                if (msgEl) {
                    ...
                    msgEl.outerHTML = `
                        <div id="social-msg-${messageId}" class="flex justify-end mb-3">
                            <div class="max-w-[80%]">
                                <div class="p-2 rounded-2xl bg-white/5 border border-white/10">
                                    <span class="text-textMuted/60 text-sm italic">${t('messages.recalledByMe')}</span>
                                </div>
                ...
                throw new Error(data.error || data.detail || (t('messages.recallFailed')));
            }
        } catch (e) {
            ...
            if (typeof showToast === 'function') {
                ...
                alert(e.message || (t('messages.recallFailed')));
            }
    ...
    hideMessage: async function (messageId, btnElement) {
        if (!confirm(t('friends.hideMessageConfirm'))) return;
        ...
        try {
            ...
            if (typeof showToast === 'function') {
                ...
                alert(e.message || (t('friends.deleteFailed')));
            }
            if (btnElement) btnElement.disabled = false;
        }
    },
    ...
    sendMessage: async function (e) {
        ...
        try {
            ...
            if (data.success) {
                ...
                if (container && data.message) {
                    ...
                    container.insertAdjacentHTML(
                        'beforeend',
                        this.renderMessageBubble(data.message)
                    );
                    container.scrollTop = container.scrollHeight;
                }
            ...
            if (typeof showToast === 'function') {
                ...
                alert(err.message || (t('messages.sendFailed')));
            }
        } finally {
            if (btn) {
                btn.disabled = false;
