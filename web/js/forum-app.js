...
const ForumApp = {
    ...
    async loadBoards() {
        try {
            ...
            // Ê∏≤Ê??ãÊùø?óË°® (Â¶ÇÊ??âÁ?Ë©?
        } catch (e) {
            console.error('Error loading boards:', e);
            if (typeof showToast === 'function') showToast('?ãÊùøËºâÂÖ•Â§±Ê?ÔºåË?Á®çÂ??çË©¶', 'error');
        }
    },
    ...
    async loadPosts(filters = {}) {
        ...
        try {
            ...
            console.error(e);
            container.innerHTML = '<div class="text-center py-10 text-danger">ËºâÂÖ•Â§±Ê?</div>';
        }
    },
    ...
    async loadTrendingTags() {
        ...
        try {
            ...
            if (container) {
                container.innerHTML = '<div class="text-sm text-danger py-1">Ê®ôÁ±§ËºâÂÖ•Â§±Ê?</div>';
            }
        }
    },
    ...
    async initPostPage() {
        ...
        // Á∂ÅÂ??âÈ?‰∫ã‰ª∂ - ‰ΩøÁî®?ãÈ??øÊ?Ê≥ïÈò≤Ê≠¢È?Ë§áÁ?ÂÆ?
    ...
    async loadPostDetail(id) {
        try {
            ...
            // ‰øùÂ??∂Â??áÁ?Â∞çË±°ÔºåÁî®?ºÂ?Á∫åÊ™¢?•Ô?Â¶ÇÊ?Ë≥ûÊ?Ê™¢Êü•‰ΩúËÄÖÔ?
            ...
            // ÂÆâÂÖ®?∞Ê∏≤??Markdown ?ßÂÆπÔºàÈò≤Ê≠?XSSÔº?
            const contentContainer = document.getElementById('post-content');
            if (typeof SecurityUtils !== 'undefined') {
                // ‰ΩøÁî® SecurityUtils ÂÆâÂÖ®Ê∏≤Ê?
            ...
            // È°ØÁ§∫‰ΩúËÄÖÊ?‰ΩúÊ??ïÔ?Á∑®ËºØ/?™Èô§Ôº?
    ...
    async loadComments(postId) {
        ...
        try {
            ...
            if (comments.length === 0) {
                container.innerHTML = '<div class="text-center text-textMuted py-4">?´ÁÑ°?ûË?</div>';
                return;
            }

            comments.forEach((comment) => {
                if (comment.type !== 'comment') return; // ?™È°ØÁ§∫‰??¨Â?Ë¶?
            ...
            if (container) {
                container.innerHTML =
                    '<div class="text-center py-4 text-danger">Ë©ïË?ËºâÂÖ•Â§±Ê?ÔºåË?Á®çÂ??çË©¶</div>';
            }
        }
    },

    async handlePush(postId) {
        if (!AuthManager.currentUser) return showToast('Ë´ãÂ??ªÂÖ•', 'warning');
    ...
    async submitReply(postId) {
        // ?≤ÈÄ??‰øùË≠∑ÔºöÂ??úÊ≠£?®Ê?‰∫§‰∏≠ÔºåÁõ¥?•Ë???
        if (this.isSubmittingReply) {
            window.APP_CONFIG?.DEBUG_MODE &&
                console.log('[submitReply] ?ö´ Already submitting, ignoring duplicate click');
            return;
        }
        ...
        try {
            ...
            showToast('Ë©ïË??ºÈÄÅÊ???, 'success');
        } catch (e) {
            showToast(e.message, 'error');
        } finally {
            // ?¢Âæ©?âÈ??Ä?ã‰∏¶Ê∏ÖÈô§?ê‰∫§‰∏≠Ê?Ë™?
            this.isSubmittingReply = false;
            if (submitBtn) {
                submitBtn.disabled = false;
                submitBtn.innerHTML =
                    '<div class="flex items-center gap-2 justify-center"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg><span>?ÅÂá∫Ë©ïË?</span></div>';
            }
        }
    },
    ...
    async handleDelete(postId) {
        if (!AuthManager.currentUser) {
            return showToast('Ë´ãÂ??ªÂÖ•', 'warning');
        }

        // Á¢∫Ë??™Èô§
        const confirmed = await showConfirm({
            title: 'Á¢∫Ë??™Èô§',
            message: 'Á¢∫Â?Ë¶ÅÂà™?§ÈÄôÁ??áÁ??éÔ?\n?™Èô§ÂæåÂ??°Ê??¢Âæ©??,
            type: 'warning',
            confirmText: 'Á¢∫Ë??™Èô§',
            cancelText: '?ñÊ?',
        });
        ...
        try {
            ...
            showToast('?™Èô§Â§±Ê?: ' + e.message, 'error');
            if (btnElement) {
    ...
    async handleEdit(postId) {
        ...
        // TODO: ÂØ¶ÁèæÁ∑®ËºØ?üËÉΩ - ?Ø‰ª•?µÂª∫‰∏Ä?ãÁ∑®ËºØÊ®°?ãÊ??ñÂ??™Âà∞Á∑®ËºØ?ÅÈù¢
        showToast('Á∑®ËºØ?üËÉΩ?ãÁôº‰∏?, 'info');
    },
    ...
    async handleTip(postId) {
        ...
        const confirmed = await showConfirm({
            title: 'Á¢∫Ë??ìË?',
            message: isPi
                ? `Á¢∫Ë??ìË? ${tipAmount} Pi Áµ¶‰??ÖÔ?\nÂ∞áÊ??ãÂ? Pi ?Ø‰?ÊµÅÁ??Ç`
                : `Á¢∫Ë??ìË? ${tipAmount} Pi Áµ¶‰??ÖÔ?\nÔºàÊ∏¨Ë©¶Ê®°ÂºèÔ???Pi Browser ?∞Â?Ôºâ`,
            type: 'info',
            confirmText: 'Á¢∫Ë??ìË?',
            cancelText: '?ñÊ?',
        });
        ...
        try {
            ...
            if (isPi && window.Pi) {
                ...
                try {
                    ...
                    showToast('?Ø‰?Ê¨äÈ?‰∏çË∂≥ÔºåË??çÊñ∞?ªÂÖ•', 'error');
                    return;
                }
                ...
                const loadingToast = showToast('Ê≠?ú®?ïÁ??Ø‰?...', 'info', 0);
                ...
                if (paymentError) {
                    showToast(paymentError === 'CANCELLED' ? '?Ø‰?Â∑≤Â?Ê∂? : '?Ø‰?Â§±Ê?', 'warning');
                    return;
                }

                if (!txHash) {
                    showToast('?Ø‰?Ë∂ÖÊ?ÔºåË??çË©¶', 'warning');
                    return;
                }
            } else {
            ...
            showToast('?ìË??êÂ?ÔºÅÊ?Ë¨ùÊÇ®?ÑÊîØ??, 'success');
            this.loadPostDetail(postId);
        } catch (e) {
            showToast('?ìË?Â§±Ê?: ' + e.message, 'error');
        }
    },
    ...
    initCreatePage() {
        ...
        const updateUIForMembership = async () => {
            ...
            try {
                ...
                if (limitsData.success) {
                    ...
                    // ‰ΩøÁî®ÂæåÁ´ØËøîÂ??ÑÈ??∂Ô?fallback ‰ΩøÁî®?ïÊ?ËºâÂÖ•?ÑÈ?ÁΩ?
                ...
                if (limitDisplay) {
                    if (error?.status === 401) {
                        limitDisplay.innerHTML =
                            '<span class="text-warning text-xs">?ªÂÖ•Â∑≤È??üÔ?Ë´ãÈ??∞Êï¥?ÜÈ???/span>';
                    } else {
                        limitDisplay.innerHTML =
        ...
        const initCharCounters = () => {
            ...
            // ÂæûÂ?Á´ØÈ?ÁΩÆÁç≤?ñÈ??∂Ô??áÂ?Á´Ø‰??Å‰??¥Ô?
            const MAX_TITLE = 200;
        ...
        document.getElementById('post-form')?.addEventListener('submit', async (e) => {
            ...
            const postAmount = getPrice('create_post');
            if (postAmount === null) {
                showToast('?πÊ†º?çÁΩÆËºâÂÖ•Â§±Ê?ÔºåË??çÊñ∞?¥Á??ÅÈù¢', 'error');
                resetButton();
                return;
            }
                ...
                try {
                        ...
                        if (postLimit.limit !== null && postLimit.remaining <= 0) {
                            ...
                            modal.innerHTML = `
                                            <div class="bg-surface w-full max-w-sm p-6 rounded-3xl border border-white/10 shadow-2xl animate-scale-in text-center">
                                                ...
                                                <h3 class="text-xl font-bold text-secondary mb-2">?ºÊ?È°çÂ∫¶Â∑≤Êªø</h3>
                                                <div class="text-textMuted text-sm mb-6 leading-relaxed">
                                                    ‰ªäÊó•Â∑≤ÁôºÂ∏?<span class="text-textMain font-bold text-base">${postLimit.count}</span> / <span class="text-textMain font-bold text-base">${postLimit.limit}</span> ÁØáÊ?Á´?br>
                                                    <span class="opacity-70">?áÁ? Premium ?ÉÂì°?≥ÂèØ?°È??ºÊ?Ôº?/span>
                                                </div>
                                                <div class="flex flex-col gap-3">
                    ...
                    if (error?.status === 401) {
                        if (typeof showToast === 'function')
                            showToast('?ªÂÖ•Â∑≤È??üÔ?Ë´ãÈ??∞Êï¥?ÜÈ??¢Â??ªÂÖ•', 'error');
                        resetButton();
                        return;
                    } else {
                        if (typeof showToast === 'function')
                            showToast('?°Ê?È©óË??ºÊ??êÂà∂ÔºåË?Á®çÂ??çË©¶', 'warning');
                        // ?ûË?Ë≠âÈåØË™§Ô?Â¶ÇË??ÇÔ?ÔºåÂ?Ë®±ÁπºÁ∫åÁî±‰º∫Ê??®Á´Ø?∑Ë??êÂà∂
                    }
                }
            }
            ...
            if (isProMember) {
                ...
                // ?úÈçµ‰øÆÂæ©ÔºöÊ™¢?•Á?ÂØ¶Á? Pi Browser UAÔºåËÄåÈ??ÖÊ™¢??SDK Â≠òÂú®
                ...
                try {
                    if (hasPiPaymentSdk) {
                        window.APP_CONFIG?.DEBUG_MODE &&
                            console.log('[CreatePost] ?í≥ Real Pi Browser - Starting payment...');
                        try {
                        ...
                        if (paymentError) {
                            showToast(
                                paymentError === 'CANCELLED' ? '?Ø‰?Â∑≤Â?Ê∂? : '?Ø‰?Â§±Ê?',
                                'warning'
                            );
                            resetButton();
                            return;
                        ...
                        if (!txHash) {
                            ...
                            showToast('?Ø‰?Ë∂ÖÊ??ñÁ??ãÁï∞Â∏∏Ô?Ë´ãËÅØÁπ´ÁÆ°?ÜÂì°', 'warning');
                            resetButton();
                            return;
                        }
                    ...
                    showToast('?Ø‰??éÁ?‰∏≠Áôº?üÈåØË™?, 'error');
                    resetButton();
                    return;
                }
            }
            ...
            try {
                ...
                successModal.innerHTML = `
                                <div class="bg-surface w-full max-w-sm p-6 rounded-3xl border border-white/10 shadow-2xl animate-scale-in text-center">
                                    ...
                                    <h3 class="text-xl font-bold text-secondary mb-2">?ºÂ??êÂ?Ôº?/h3>
                                    <div class="text-textMuted text-sm mb-6">
                                        ?®Á??áÁ?Â∑≤Ê??ü‰??àÂÑ≤Â≠ò„Ä?br>
                                        <span class="text-primary animate-pulse">Ê≠?ú®?çÂ??áÁ?Ë©≥Ê???..</span>
                                    </div>
                ...
                if (txHash && txHash !== 'pro_member_free' && !txHash.startsWith('mock_')) {
                    // È°ØÁ§∫?ãÂ•Ω?ÑÈåØË™?Modal ?åÈ? alert
                    ...
                    errorModal.innerHTML = `
                        <div class="bg-surface w-full max-w-md p-6 rounded-3xl border border-white/10 shadow-2xl animate-scale-in">
                            ...
                            <h3 class="text-xl font-bold text-secondary mb-3 text-center">?ºÊ?Â§±Ê?‰ΩÜÊîØ‰ªòÂ∑≤ÂÆåÊ?</h3>
                            <div class="text-textMuted text-sm mb-4 leading-relaxed">
                                <p class="mb-2">?®Á??Ø‰?Â∑≤Ê??üÔ?‰ΩÜÊ?Á´†ÁôºÂ∏ÉÂ§±?ó„ÄÇË?‰øùÂ?‰ª•‰?‰∫§Ê? ID ‰∏¶ËÅØÁπ´ÂÆ¢?çË??ÜÔ?</p>
                                <div class="bg-background/50 p-3 rounded-xl border border-white/10 mb-3">
                                    <div class="text-xs text-textMuted mb-1">‰∫§Ê? ID</div>
                                    <div class="text-textMain font-mono text-xs break-all" id="error-txhash">${SecurityUtils.escapeHTML(txHash || '')}</div>
                                </div>
                                <p class="text-xs opacity-60">?ØË™§Ë®äÊÅØÔº?{SecurityUtils.escapeHTML(err.message || '')}</p>
                            </div>
                    ...
                    showToast('?ºÂ?Â§±Ê?: ' + err.message, 'error');
                }
                resetButton();
            }
        });
    ...
    async initDashboardPage() {
        ...
        if (avatarEl && user.username) {
            // XSS Fix: ‰ΩøÁî® textContent ?ø‰ª£ innerHTML
    ...
    async loadWalletStatus() {
        ...
        try {
            ...
            statusText.textContent = 'ËºâÂÖ•Â§±Ê?';
            statusText.classList.add('text-danger');
    ...
    async loadTransactions() {
        ...
        try {
            // ‰ΩøÁî® Promise.allSettled Á¢∫‰??®Â? API Â§±Ê??Ç‰??ΩÈ°ØÁ§∫ÂèØ?®Êï∏??
            ...
            const payments = (paymentsData.payments || [])
                // ‰øùÁ? Premium ?ÉÂì°?çË≤ª?ºÊ?Ë®òÈ?‰ΩÜÊ?Ë®òÁÇ∫?çË≤ª
                .map((p) => {
    ...
    async submitReport() {
        ...
        if (!reportType) {
            return showError('Ë´ãÈÅ∏?áÈ?Ë¶èÈ???);
        }
        ...
        try {
            ...
            showToast('?âÂ†±?ê‰∫§?êÂ?ÔºåÊ??ëÊ??°Âø´ÂØ©Ê†∏', 'success');
        } catch (e) {
            showError('?ê‰∫§Â§±Ê?: ' + e.message);
        } finally {
            btn.disabled = false;
...
// Á¢∫‰???DOM ËºâÂÖ•ÂæåÂü∑Ë°åÔ??ÖÈ??®Á? forum ?ÅÈù¢ÔºåSPA ‰∏ªÈ???switchTab Ëß∏ÁôºÔº?
