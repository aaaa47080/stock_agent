...
const WalletApp = {
    ...
    async loadData() {
        ...
        try {
            ...
            // We use Promise.allSettled to ensure one failure doesn't block everything
            ...
            this.log('API Response received', results);
            ...
            if (results[0].status === 'rejected') this.log('Payments fetch failed', results[0].reason);
            ...
            // Combine & Sort
            const allTx = [...payments, ...tipsSent, ...tipsReceived]
                .sort((a, b) => new Date(b.created_at) - new Date(a.created_at));

            this.log(`Processed ${allTx.length} transactions`);

            // Calculate Totals
            const totalOut = allTx.filter(t => t.amount < 0).reduce((acc, t) => acc + Math.abs(t.amount), 0);
            const totalIn = allTx.filter(t => t.amount > 0).reduce((acc, t) => acc + t.amount, 0);
            ...
            allTx.forEach(tx => {
                ...
                el.className = 'flex items-center justify-between border-b border-white/5 py-4 hover:bg-white/5 px-3 rounded-2xl transition cursor-pointer last:border-0 group';
