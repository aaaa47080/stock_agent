...
class MockLLM:
    ...
    def _make_response(self, prompt: str) -> AIMessage:
        ...
        if '"status"' in prompt or "json" in prompt_lower and "tasks" in prompt_lower:
            if "pi" in prompt_lower and ("price" in prompt_lower or "價格" in prompt_lower or "多少" in prompt_lower):
                payload = {
                    "status": "ready",
...
check("crypto agent 有 get_pi_price",   "get_pi_price"        in tool_names, f"工具數={len(tool_names)}")
...
check("intent_understanding 有 status",  bool(intent.get("status")),  intent.get("status", "missing"))
check("PI 查詢路由到 crypto agent",       "crypto" in agents_used,     f"agents={agents_used}")
check("boundary_routed 不存在（走 LLM 路由）",
...
check("你好 → direct_response", greet_intent.get("status") == "direct_response",
...
print(f"\n{'='*60}")
print(f"  測試結果：{PASS} {passed}/{total} 通過  {'❌ ' + str(failed) + ' 失敗' if failed else ''}")
print(f"{'='*60}")
