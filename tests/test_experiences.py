...
def test_retrieve_relevant_returns_empty_on_db_error():
    ...
    store = ExperienceStore()
    with patch(
        "core.database.experiences.DatabaseBase.query_all",
...
def test_retrieve_relevant_filters_by_user_id():
    ...
    with patch(
        "core.database.experiences.DatabaseBase.query_all", side_effect=fake_query_all
    ):
        store.retrieve_relevant(
            user_id="u-specific", task_family="crypto", query="BTC", llm=None
        )
...
def test_layer3_skipped_when_fewer_than_2_candidates():
    ...
    with patch("core.database.experiences.DatabaseBase.query_all", return_value=rows):
        store.retrieve_relevant(
            user_id="u1", task_family="crypto", query="BTC", llm=mock_llm
        )
