...
class TestDeletePost:
    """Tests for delete_post audit integration."""

    def test_delete_post_uses_central_audit_logger(self):
        """Successful post deletion should go through AuditLogger instead of raw SQL."""
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
        ...
        with patch("core.database.forum.get_connection", return_value=mock_conn):
            with patch("core.database.forum.AuditLogger.log") as mock_audit_log:
                result = delete_post(42, "user-123")

        assert result is True
        mock_audit_log.assert_called_once()
        audit_kwargs = mock_audit_log.call_args.kwargs
        assert audit_kwargs["action"] == "delete_post"
        assert audit_kwargs["cursor"] is mock_cursor
        assert audit_kwargs["method"] == "DELETE"
