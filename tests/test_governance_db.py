...
class TestReportManagement:
    ...
    @patch("core.database.governance.reports.check_daily_report_limit")
    @patch("core.database.governance.reports.get_user_membership")
    def test_create_report_limit_exceeded(
        self, mock_membership, mock_check_limit, mock_get_author
    ):
    ...
    @patch("core.database.governance.reports.check_daily_report_limit")
    @patch("core.database.governance.reports.get_user_membership")
    def test_create_report_own_content(
        self, mock_membership, mock_check_limit, mock_get_author
    ):
    ...
    def test_create_report_duplicate(
        self, mock_get_conn, mock_membership, mock_check_limit, mock_get_author
    ):
...
class TestVoting:
    ...
    def test_vote_on_report_approve(
        self, mock_get_conn, mock_reputation, mock_membership, mock_activity_conn
    ):
        ...
        mock_cursor.fetchone.side_effect = [
            (
                "pending",
                "reporter-user-id",
            ),  # Report status with reporter_user_id (2 columns)
    ...
    @patch("core.database.governance.voting.get_audit_reputation")
    @patch("core.database.governance.voting.get_connection")
    def test_vote_on_report_already_voted(
        self, mock_get_conn, mock_reputation, mock_membership
    ):
    ...
    def test_finalize_report_approved(
        self,
        mock_update_reputation,
        mock_get_votes,
        mock_determine_action,
        mock_get_violation_points,
        mock_apply_suspension,
        mock_add_violation,
        mock_get_author,
        mock_get_conn,
    ):
    ...
    @patch("core.database.governance.voting.get_report_votes")
    @patch("core.database.governance.voting.update_audit_reputation")
    def test_finalize_report_rejected(
        self, mock_update_reputation, mock_get_votes, mock_get_author, mock_get_conn
    ):
...
class TestViolations:
    ...
    def test_add_violation_points(self, mock_get_conn, mock_activity_conn):
        ...
        result = add_violation_points(
            None, "test-user", 3, "medium", "spam", "report", 1, "admin-user"
        )
