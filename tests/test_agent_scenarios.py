...
class TestAgentScenarios(unittest.TestCase):
    ...
    @pytest.mark.skip(
        reason="_classify removed; test needs rework for new graph-based API"
    )
        ...
        for i, case in enumerate(SCENARIOS):
            ...
            with patch.object(self.manager, "_classify", return_value=mock_class):
                ...
                try:
                    ...
                    self.assertEqual(
                        sorted(context.topics),
                        sorted(expected_topics),
                        f"Topics mismatch: {context.topics} != {expected_topics}",
                    )
                    ...
                    self.assertEqual(
                        context.intent,
                        case.get("intent"),
                        f"Intent mismatch: {context.intent} != {case.get('intent')}",
                    )
