...
class TestSanitizeUrlPresence:
    ...
    def test_sanitize_url_blocks_javascript_protocol(self, filepath):
        content = _read_js_file(filepath)
        match = re.search(r"function sanitizeUrl\s*\((\w+)\)\s*\{([^}]+)\}", content, re.DOTALL)
        assert match is not None, f"{filepath}: sanitizeUrl function not found"
        body = match.group(2)
        assert "javascript:" in body.lower(), f"{filepath}: sanitizeUrl does not check for javascript: protocol"
    ...
    def test_sanitize_url_blocks_data_protocol(self, filepath):
        ...
        body = match.group(2)
        assert "data:" in body.lower(), f"{filepath}: sanitizeUrl does not check for data: protocol"
    ...
    def test_href_uses_sanitize_url(self, filepath):
        ...
        for pattern in href_patterns:
            ...
            if not is_safe:
                ...
                prop_match = re.search(rf'sanitizeUrl\(\s*{re.escape(expr)}\s*\)', content)
...
class TestSafetyTabClipboardFix:
    ...
    def test_uses_data_attribute_or_encodeURIComponent(self):
        ...
        has_safe_pattern = (
            "data-clipboard" in content
            or "encodeURIComponent" in content
        )
...
class TestAIContentEscaping:
    ...
    @pytest.mark.parametrize("filepath,field", [
