...
class TestScamReportCreation:
    ...
    @patch("core.database.scam_tracker.AuditLogger.log")
    def test_create_report_success(
        self, mock_audit_log, mock_membership, mock_sys_config_conn, mock_get_connection
    ):
        ...
        assert mock_audit_log.call_args.kwargs["action"] == "create_scam_report"
        assert mock_audit_log.call_args.kwargs["cursor"] is mock_cursor
...
class TestScamReportQueries:
    ...
    def test_search_wallet_found(
        self, mock_get_report, mock_validate, mock_get_connection
    ):
...
class TestComments:
    ...
    def test_add_comment_pro_user(
        self, mock_validate, mock_membership, mock_get_connection, mock_sys_config_conn
    ):
        ...
        result = add_scam_comment(
            1, "user-001", "This is a testimony about the scammer."
        )
    ...
    @patch("core.database.scam_tracker.get_connection")
    @patch("core.database.scam_tracker.get_user_membership")
    def test_add_comment_non_premium_user(
        self, mock_membership, mock_get_connection, mock_sys_config_conn
    ):
