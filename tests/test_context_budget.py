...
def _make_compact(goal="查詢BTC", progress="取得價格", open_q="", next_s=""):
    ...
    return CompactPrompt(
        goal=goal, progress=progress, open_questions=open_q, next_steps=next_s
    )
...
def test_long_history_over_budget():
    from core.agents.context_budget import CONTEXT_CHAR_BUDGET, history_exceeds_budget
...
def test_format_compact_state_includes_all_sections():
    ...
    cp = _make_compact(
        goal="查詢BTC", progress="已取得價格", open_q="技術指標待確認", next_s="查詢RSI"
    )
...
def test_manager_uses_compact_state_when_over_budget():
    ...
    with patch("core.agents.manager.history_exceeds_budget", return_value=True):
        with patch(
            "core.agents.manager._read_compact_for_manager", return_value=mock_compact
        ):
            from core.agents.manager import _get_history_for_prompt

            result = _get_history_for_prompt(
                long_history, user_id="u1", session_id="s1"
            )
