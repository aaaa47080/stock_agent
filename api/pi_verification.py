...
async def verify_pi_access_token(
    access_token: str, expected_pi_uid: str
) -> Dict[str, Any]:
    """
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 401:
                ...
                raise HTTPException(
                    status_code=401, detail="Invalid or expired Pi Access Token"
                )

            if response.status_code != 200:
                logger.error(
                    f"Pi API returned unexpected status: {response.status_code}"
                )
                raise HTTPException(
                    status_code=502, detail="Pi verification service error"
                )
        ...
        raise HTTPException(
            status_code=504, detail="Pi verification service timeout - please try again"
        )
    except httpx.HTTPError as e:
        logger.error(f"Pi API HTTP error: {e}")
        raise HTTPException(
            status_code=502, detail="Unable to connect to Pi verification service"
        )
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error during Pi verification"
        )
...
async def verify_pi_payment_token(access_token: str, payment_id: str) -> Dict[str, Any]:
    ...
    if not PI_API_KEY:
        raise HTTPException(
            status_code=500, detail="Server configuration error: Pi API not configured"
        )
