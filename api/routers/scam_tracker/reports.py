...
async def search_scam_wallet(
    ...
    try:
        ...
        if report:
            ...
            return {"success": True, "found": False, "message": "該錢包未被舉報"}
    ...
    try:
        ...
        if result.get("success"):
            ...
            if error == "premium_membership_required":
                ...
                raise HTTPException(
                    status_code=429, detail=f"已達每日舉報上限 ({used}/{limit})"
                )
                ...
                raise HTTPException(
                    status_code=400, detail=f"可疑錢包地址無效: {detail}"
                )
            elif error == "invalid_reporter_wallet":
                raise HTTPException(
                    status_code=400, detail=f"舉報者錢包地址無效: {detail}"
                )
