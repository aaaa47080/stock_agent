...
Follows the same pattern as api/routers/jpstock.py
"""
...
async def _fetch_quote_v8(client: httpx.AsyncClient, symbol: str) -> dict | None:
    ...
    try:
        ...
        logger.warning(f"[instock] quote failed {symbol}: {e}")
        return None
...
async def _fetch_technicals_yahoo(symbol: str) -> dict:
    ...
    try:
        ...
        logger.warning(f"[instock] technicals fetch failed {symbol}: {e}")
        return {}
    ...
    try:
        ...
        logger.warning(f"[instock] technicals parse failed {symbol}: {e}")
        return {}
...
async def get_in_market(symbols: Optional[str] = None):
    targets = (
        ...
        if symbols else DEFAULT_IN_SYMBOLS
    )
...
async def get_in_pulse(
    ...
    if not quotes:
        raise HTTPException(status_code=404, detail=f"查無印度股代號「{sym}」或目前無法獲取數據")
...
async def get_in_klines(symbol: str, interval: str = "1d", limit: int = 200):
    sym = _normalise(symbol)
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"無法獲取印度股歷史數據: {e}")
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"解析印度股歷史數據失敗: {e}")
