...
Follows the same pattern as api/routers/instock.py
"""
...
async def _fetch_quote_v8(client: httpx.AsyncClient, symbol: str) -> dict | None:
    ...
    try:
        ...
        price = round(float(meta["regularMarketPrice"]), 0)
        ...
        logger.warning(f"[krstock] quote failed {symbol}: {e}")
        return None
...
async def _fetch_technicals_yahoo(symbol: str) -> dict:
    ...
    try:
        ...
        logger.warning(f"[krstock] technicals fetch failed {symbol}: {e}")
        return {}
    ...
    try:
        ...
        high_52w = round(max(highs), 0) if highs else None
        low_52w  = round(min(lows),  0) if lows  else None

        return {"rsi": rsi, "52w_high": high_52w, "52w_low": low_52w}
    except Exception as e:
        logger.warning(f"[krstock] technicals parse failed {symbol}: {e}")
        return {}
...
async def get_kr_market(symbols: Optional[str] = None):
    targets = (
        ...
        if symbols else DEFAULT_KR_SYMBOLS
    )
...
async def get_kr_pulse(
    ...
    if not quotes:
        raise HTTPException(status_code=404, detail=f"查無韓股代號「{sym}」或目前無法獲取數據")
    ...
    key_points = [
        ...
        f"52W High: {int(tech['52w_high']):,} {currency}" if isinstance(tech.get('52w_high'), (int, float)) else "52W High: N/A",
        f"52W Low: {int(tech['52w_low']):,} {currency}" if isinstance(tech.get('52w_low'), (int, float)) else "52W Low: N/A",
...
async def get_kr_klines(symbol: str, interval: str = "1d", limit: int = 200):
    sym = _normalise(symbol)
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"無法獲取韓股歷史數據: {e}")
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"解析韓股歷史數據失敗: {e}")
