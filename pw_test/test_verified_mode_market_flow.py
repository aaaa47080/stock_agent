...
async def run_verified_mode_market_flow_test():
    ...
    async with async_playwright() as p:
        ...
        assert research_option_disabled is False
...
async def run_research_mode_market_flow_test():
    from playwright.async_api import async_playwright
    ...
    async with async_playwright() as p:
        ...
        async def handle_route(route):
            ...
            if url.endswith("/api/config"):
                await route.fulfill(status=200, content_type="application/json", body=json.dumps({"test_mode": False, "current_settings": {}}))
                return
            if url.endswith("/api/model-config"):
                await route.fulfill(status=200, content_type="application/json", body=json.dumps({"model_config": {}}))
                return
            if url.endswith("/api/user/api-keys"):
                await route.fulfill(
                    status=200,
                    content_type="application/json",
                    body=json.dumps({"keys": {"openai": {"has_key": True, "masked_key": "sk-***test", "model": "gpt-4o-mini"}}}),
                )
                return
            if url.endswith("/api/user/api-keys/openai/full"):
                await route.fulfill(status=200, content_type="application/json", body=json.dumps({"key": "sk-test-openai"}))
                return
            if url.endswith("/api/analyze/modes"):
                await route.fulfill(
                    status=200,
            ...
            if "/api/chat/sessions" in url and route.request.method == "GET":
                ...
                return
            if "/api/chat/sessions" in url and route.request.method == "POST":
                await route.fulfill(status=200, content_type="application/json", body=json.dumps({"session_id": "sess-research-001", "title": "New Chat"}))
                return
            if url.endswith("/api/analyze") and route.request.method == "POST":
                raw_payload = route.request.post_data or "{}"
                analyze_requests.append(json.loads(raw_payload))
                await route.fulfill(
                    ...
                    headers={"Content-Type": "text/event-stream"},
                    body=(
                        'data: {"content":"研究模式已完成初步市場判定。"}\n\n'
                        'data: {"type":"response_metadata","data":{"analysis_mode":"research","verification_status":"research","used_tools":["web_search","us_stock_price"],"data_as_of":"2026-03-13T12:00:00Z","query_type":"price_lookup","resolved_market":"us","policy_path":"discovery_lookup"}}\n\n'
                        'data: {"done":true}\n\n'
                    ),
                )
                return
        ...
        await selector.select_option("research")
        await page.fill("#user-input", "tsm 現在多少？")
        await page.click("#send-btn")
        ...
        await page.wait_for_function(
            ...
            () => {
                const body = document.body.innerText || '';
                return body.includes('研究模式已完成初步市場判定。') &&
                    body.includes('模式: research') &&
                    body.includes('查詢類型: price_lookup') &&
                    body.includes('市場: us') &&
                    body.includes('路徑: discovery_lookup');
            }
        ...
        assert analyze_requests, "expected /api/analyze to be called"
        assert analyze_requests[0]["analysis_mode"] == "research"
        assert analyze_requests[0]["message"] == "tsm 現在多少？"
